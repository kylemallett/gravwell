...
const (
	...
	defaultConfigLoc  = `/opt/gravwell/etc/ubiquiti_fetcher.conf`
	defaultConfigDLoc = `/opt/gravwell/etc/ubiquiti_fetcher.conf.d`
	defaultStateLoc   = `/opt/gravwell/etc/ubiquiti_fetcher.state`
	defaultLogLoc     = `/opt/gravwell/log/ubiquiti_fetcher.log`

	appName      string = `Ubiquiti Fetcher`
	ingesterName string = "ubiquiti_fetcher"
)
...
type cfgType struct {
	...
	UbiquitiConf map[string]*UbiquitiConf
}
...
func (c cfgType) Verify() error {
	...
	// ensure there is at least one backend target
...
func GetConfig(path, overlayPath string) (*cfgType, error) {
	...
	if err := c.UbiquitiVerify(); err != nil {
		return nil, err
	}
	...
	if c.Global.Log_File == `` {
		c.Global.Log_File = defaultLogLoc
	}
	if c.Global.RequestPerMinute == `` {
		c.Global.RequestPerMinute = strconv.Itoa(defaultRequestPerMinute)
	}
...
func (c *cfgType) IngestBaseConfig() config.IngestConfig { return c.Global.IngestConfig }
func (c *cfgType) AttachConfig() attach.AttachConfig      { return c.Attach }
func (c *cfgType) VerifyRemote() bool                     { return c.Global.Verify_Remote_Certificates }
